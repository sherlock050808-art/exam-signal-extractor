# Exam Signal Extractor（考试信号提取器）

> 版本：v2.0（v3.0 预览版见 [`portfolio-v3/`](portfolio-v3/)）  
> 核心升级：从“怎么做复习资料”重新定义为“**如何从海量教学信息中提取对考试最有预测价值的信号**”。  
> v3.0 预览：将**考试意图处理**与**考试内容结构化**分离，用 Knowledge Node 作为可查询入口，同时复用经 v2.0 验证的输出模板。
...
| 版本 | 定位 | 位置 |
|------|------|------|
| v1.0 | 信息蒸馏 SOP（历史版本） | [`docs/methodology/information-distillation-sop.md`](docs/methodology/information-distillation-sop.md) |
| v2.0 | 从课堂录音提取考试信号；七层考试地图；开卷/闭卷策略 | [`docs/SKILL.md`](docs/SKILL.md) 与 [`docs/methodology/`](docs/methodology/) |
| **v3.0（预览）** | 考试意图与考试内容分离处理；Knowledge Node 作为可查询入口；复用 v2.0 输出模板 | [`portfolio-v3/`](portfolio-v3/) |

v3.0 **不替代** v2.0，而是在其基础上增加结构化数据层，使同一套方法论可以在多门课程、多个考试周期中持续维护。
...
### 方式一：让 AI 读取 v3.0 Skill 文档（推荐需要长期维护/多课程复用）

把 [`portfolio-v3/SKILL.md`](portfolio-v3/SKILL.md) 提供给 Kimi / Claude 等支持长上下文的助手，然后说：

> “按这个帮我准备《XXX》考试，这是课堂录音转录文本、课件和考点大纲。”

AI 会分离考试意图处理与考试内容结构化，建立 Knowledge Node 知识树，再生成闭卷或开卷资料。

### 方式二：让 AI 读取 v2.0 Skill 文档（推荐单次快速备考）
...
├── LICENSE                                      # MIT 许可证
├── .gitignore                                   # Git 忽略规则
├── portfolio-v3/                                # v3.0 预览：意图/内容分离 + Knowledge Node
│   ├── README.md                                # v3.0 概览
│   ├── SKILL.md                                 # 可直接给 AI 读取的 v3.0 Skill 文档
│   ├── references/                              # v3.0 方法论及 v2.0/v1.0 参考资料
│   ├── templates/                               # v3.0 节点 schema 与闭卷生成工作流模板
│   ├── examples/                                # v3.0 完整案例（数控技术）
│   └── checklists/                              # 考前/考后检查清单（v2.0）
├── docs/
